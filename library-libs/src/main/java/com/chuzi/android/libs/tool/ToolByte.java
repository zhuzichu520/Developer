package com.chuzi.android.libs.tool;
...
public class ToolByte {
    ...
    public static byte[] hexStringToBytes(String hexString) {
        ...
        for (int i = 0; i < length; i++) {
            int pos = i * 2;
            d[i] = (byte) (ToolByte.charToByte(hexChars[pos]) << 4 | ToolByte.charToByte(
                    hexChars[pos + 1]));
        }
        return d;
    }
    ...
    public static byte[] stringToBytes(String s, int length) {
        byte[] src =
                ToolByte.getBytes((s == null || s.length() == 0) ? new char[0] : s.toCharArray());
    ...
    public static String bytesToAscii(byte[] bytes, int offset, int dateLen) {
        ...
        asciiStr = new String(data, StandardCharsets.ISO_8859_1);
        return asciiStr;
    }
    ...
    public static byte[] intToByteArray(int number, int length) {
        if (length == 1) {
            ...
            return new byte[]{(byte) ((number >> 8) & 0xFF),
                    (byte) (number & 0xFF)};
        } else if (length == 3) {
            return new byte[]{(byte) ((number >> 16) & 0xFF),
                    (byte) ((number >> 8) & 0xFF),
                    (byte) (number & 0xFF)};
        }
